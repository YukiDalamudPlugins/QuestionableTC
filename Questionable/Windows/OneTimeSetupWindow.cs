...
internal sealed class OneTimeSetupWindow : LWindow
{
    ...
    public override void DrawContent()
    {
        ...
        if (allRequiredInstalled)
        {
            ...
            {
                if (ImGuiComponents.IconButtonWithText(FontAwesomeIcon.Check, _L("Finish Setup")))
                {
        ...
        else
        {
            using (ImRaii.Disabled())
            {
                using (ImRaii.PushColor(ImGuiCol.Text, ImGuiColors.DalamudRed))
                    ImGuiComponents.IconButtonWithText(FontAwesomeIcon.Check, _L("Missing required plugins"));
            }
        }

        ImGui.SameLine();

        if (ImGuiComponents.IconButtonWithText(FontAwesomeIcon.Times, _L("Close window & don't enable Questionable")))
        {
